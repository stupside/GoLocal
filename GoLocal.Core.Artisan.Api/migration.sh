...
if [ "$option" -eq 1 ]; then
  ...
  if [ "$first" = 'y' ]; then
    ...
    dotnet ef migrations add Initial --context Context --project "../GoLocal.Core.Persistence" --output-dir "../GoLocal.Core.Persistence/EntityFramework/Migrations"
  else
    read -r -p "name > " name
    echo
    dotnet ef migrations add "$name" --context Context --project "../GoLocal.Core.Persistence" --output-dir "../GoLocal.Core.Persistence/EntityFramework/Migrations"
  fi
